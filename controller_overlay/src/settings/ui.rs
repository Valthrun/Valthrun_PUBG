...
impl SettingsUI {
    ...
    pub fn render(
        ...
        ui.window(obfstr!("Valthrun_PUBG"))
            ...
            .build(|| {
                ...
                if let Some(_tab_bar) = ui.tab_bar("main") {
                    if let Some(_tab) = ui.tab_item("Information") {
                        ...
                        ui.text(obfstr!("Valthrun_PUBG an open source external read only kernel gameplay enhancer."));
                        ui.text(&format!("{} Version {} ({})", obfstr!("Valthrun_PUBG"), VERSION, env!("BUILD_TIME")));
                    ...
                    if let Some(_) = ui.tab_item("Monitor") {
                        self.render_monitor_settings(&mut *settings, ui, app);
                    }
                }
            });
    }
    ...
    fn render_monitor_settings(
        &mut self,
        settings: &mut AppSettings,
        ui: &imgui::Ui,
        app: &Application,
    ) {
        ui.text("Monitor Selection");
        ui.separator();

        let monitors = ui.io().display_size[0] > 0.0; // Check if we have a display
        if monitors {
            let mut selected = settings.selected_monitor as usize;
            let monitor_count = 2; // For now hardcode to 2 since you mentioned you have 2 monitors
            if selected >= monitor_count {
                selected = 0;
